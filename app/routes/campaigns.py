...
async def list_campaigns(token: str = Depends(require_user_auth)) -> list[CampaignRead]:
    return []


@router.post("/", response_model=CampaignRead, status_code=status.HTTP_201_CREATED)
async def create_campaign(campaign: CampaignCreate, token: str = Depends(require_user_auth)) -> CampaignRead:
    return CampaignRead(id="0", **campaign.model_dump())


@router.get("/{campaign_id}", response_model=CampaignRead)
async def get_campaign(campaign_id: int, token: str = Depends(require_user_auth)) -> CampaignRead:
...
async def start_campaign(company_id: str, token: str = Depends(require_user_auth)) -> CampaignStartResponse:
